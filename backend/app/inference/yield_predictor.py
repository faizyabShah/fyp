...
def unnormalize_yield(normalized_yield):
    minimum = 0.0001437949359174
    maximum = 0.0071578947368421
...
def generate_predictions_and_save_csv(tiff_dir, model, device, output_dir, output_filename="yield_report.csv"):
    """
    ...
    """
    predicted_yield, dates = predict_yield_field(tiff_dir, model, device)
...
if __name__ == "__main__":
    tiff_folder = "./data/sentinel_data"  # Your TIFF folder path
    model = RNNModel()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.to(device)
    # Load your pre-trained model weights
    loaded_checkpoint = torch.load('data/model_weights/germany_satellite_yield_model.pth', map_location=device)
    model.load_state_dict(loaded_checkpoint['model'])
    
    output_dir = "data/output"
    generate_predictions_and_save_csv(tiff_folder, model, device, output_dir)
