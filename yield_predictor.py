...
if __name__ == "__main__":
    # Directory with yield TIFF files (each TIFF covers the whole field)
    tiff_folder = "./data/sentinel_data"
    
    model = RNNModel()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.to(device)
    loaded_checkpoint = torch.load('./data/model_wieghts/germany_satellite_yield_model.pth', map_location=device)
    model.load_state_dict(loaded_checkpoint['model'])
    
    predicted_yield, dates = predict_yield_field(tiff_folder, model, device)
